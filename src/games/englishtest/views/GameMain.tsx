      ...
      <Box align="center" valign="flex-start" style={{width:'100%', height:'100%'}}>
        ...
        <Box flexDirection="column" valign="flex-start" style={{width:'800px', height:'100%', background:'#ffefef'}}>
          ...
          <Box flexDirection="column" style={{height:'calc(100% - 200px)', background:'#fdfdfd'}}>
            {
            data.current[index]?<>
            <Input readonly={true} name="현재형" formKey="curr" value={data.current[index].curr} />
          ...
          <Box align="space-between" style={{height:'100px'}}>
            ...
            {data.current[index] && <Button name="정답확인" onClick={confirm}></Button>}
          </Box>
...
function Box({flexDirection='row', align='flex-start', valign='center', textSize = 3, textColor = 'black', style, children}:React.PropsWithChildren<BoxProps>){
  return <div style={{width:'100%', display:'flex', flexDirection:flexDirection, alignItems:valign, justifyContent:align, padding:'10px',
  fontSize:ColorMap.get(textSize), fontWeight:600, color:textColor,
   ...style}}>{children}</div>
}
...
function Input({name, readonly=false, value, formKey, formObject, style}:InputProps){
  ...
  return <Box align="space-between">
    {name && <Box>{name}</Box>}
    <input ref={ref} disabled={readonly} readOnly={readonly} style={{...style}} value={value} onChange={(e)=>{

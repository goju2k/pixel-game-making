...
export class BoxCollider {
  ...
  //박스 설정
  width:number=0;height:number=0;offsetX:number=0;offsetY:number=0;

  //박스 영역
  x1:number=0;y1:number=0;x2:number=0;y2:number=0;
  ...
  checkCollisionList(targetList:ObjectBase[]) {
    for(let target of targetList){
      ...
      if(this.checkCollisionWith(target)) return true;
    }
    return false
  }
